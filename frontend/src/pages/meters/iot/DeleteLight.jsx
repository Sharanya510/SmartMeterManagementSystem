...
import { deleteLight, findLightById } from '../../../services/deviceApi';
...
const DeleteLight = () => {
  ...
  const handleDeleteDevice = async (id) => {
    try {
      const res = await deleteLight(id);
      if (res.status === 200) {
  ...
  return (
    <div className='mt-24'>
      <p className='font-bold text-xl p-1 px-4 text-center my-4'>
        Do you want to delete {device.lightName}?
      </p>
      ...
      <div className='flex justify-center items-center mt-4 gap-8'>
        ...
        <button
          onClick={() => navigate('/client/devices/iot/lights')}
